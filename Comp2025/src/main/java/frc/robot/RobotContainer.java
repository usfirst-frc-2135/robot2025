...
public class RobotContainer {
  ...
  // private final SwerveRequest.RobotCentric aim = new
  ...
  public RobotContainer() {
    ...
    facing.HeadingController = new PhoenixPIDController(kHeadingKp, kHeadingKi, kHeadingKd); // Swerve steer PID for
                                                                                             // facing swerve request
    // Identify the field
  ...
  /****************************************************************************
   * 
   * Callbacks used by dashboard autonomous choosers to reload when an onChange
   * event occurs
   */
  ...
  private void addDashboardWidgets() {
    ...
    // Command tab
    // SmartDashboard.putData("PrepareToClimb", new PrepareToClimb(m_climber,
    // m_feeder));
  ...
  private void configureButtonBindings() {
    ...
    // Operator - A, B, X, Y
    //
    m_operatorPad.a().onTrue(m_manipulator.getCalibrateCommand().ignoringDisable(true)); // TODO: manual wrist
                                                                                         // calibration command
    ...
    // 5, rightTrigger = 4}
    //
    m_operatorPad.leftTrigger(Constants.kTriggerThreshold)
        .onTrue(new ScoreAlgae(m_elevator, m_manipulator, m_led, m_hid));
    m_operatorPad.rightTrigger(Constants.kTriggerThreshold)
        .onTrue(new ScoreCoral(m_elevator, m_manipulator, m_led, m_hid));
  ...
  private void initDefaultCommands() {
    ...
    m_manipulator.setDefaultCommand(
        m_manipulator.getHoldPositionCommand(ClawMode.CORALMAINTAIN, m_manipulator::getCurrentAngle));

    // Default command - manual mode
    // m_elevator.setDefaultCommand(m_elevator.getJoystickCommand(( ) ->
    // getElevatorAxis( )));
    // m_manipulator.setDefaultCommand(m_manipulator.getJoystickCommand(( ) ->
    // getWristAxis( )));
  }
  ...
  public Command getAutonomousCommand() {
    ...
    DataLogManager
        .log(String.format("getAuto: autoMode %s startOption %s (%s)", autoKey, startPose, m_autoCommand.getName()));
