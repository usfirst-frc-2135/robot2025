...
public class Climber extends SubsystemBase {
  ...
  // Alerts
  private final Alert m_leftAlert = new Alert(String.format("%s: Left motor init failed!", getSubsystem()),
      AlertType.kError);
  private final Alert m_rightAlert = new Alert(String.format("%s: Right motor init failed!", getSubsystem()),
      AlertType.kError);
  ...
  public Climber() {
    ...
    boolean rightValid = PhoenixUtil6.getInstance().talonFXInitialize6(m_rightMotor, kSubsystemName + "Right",
        CTREConfigs6.climberFXConfig(false, Units.degreesToRotations(kLengthMin),
            Units.degreesToRotations(kLengthMax)));
  ...
  public void simulationPeriodic() {
    ...
    m_climberSim.setRotorVelocity(
        Conversions.inchesToWinchRotations(Units.metersToInches(m_elevSim.getVelocityMetersPerSecond()),
            kRolloutRatio));
  ...
  private void moveWithJoystick(DoubleSupplier getAxis) {
    ...
    if (newMode != m_mode) {
      m_mode = newMode;
      DataLogManager
          .log(String.format("%s: Manual move mode %s now %.1f inches %s", getSubsystem(), m_mode, m_leftLength,
  ...
  private void moveToPositionInit(double newLength, boolean holdPosition) {
    ...
    if (!(m_leftCalibrated && m_rightCalibrated)) {
      DataLogManager
          .log(String.format("%s: MM Position move target %.1f in - NOT CALIBRATED!", getSubsystem(), m_targetLength));
      return;
    }
    ...
    if (holdPosition || newLength != m_targetLength || !MathUtil.isNear(newLength, m_leftLength, kToleranceInches)) {
      ...
      DataLogManager
          .log(String.format("%s: MM Position already achieved - target %s inches", getSubsystem(), m_targetLength));
    }
  }
  ...
  private boolean moveToPositionIsFinished(boolean holdPosition) {
    ...
    if (m_mmWithinTolerance.calculate(Math.abs(error) < kToleranceInches) || timedOut || hittingHardStop) {
      ...
      if (!m_mmMoveIsFinished)
        DataLogManager
            .log(String.format(
                "%s: MM Position move finished - Current inches: %.1f (difference %.1f) - Time: %.3f sec %s",
  ...
  private boolean calibrateIsFinished() {
    ...
    if (leftCalibrated && !m_leftCalibrated)
      DataLogManager
          .log(String.format("%s: Left stalled %s (right %s)", getSubsystem(), leftCalibrated, rightCalibrated));
    if (rightCalibrated && !m_rightCalibrated)
      DataLogManager
          .log(String.format("%s: Right stalled %s (left %s)", getSubsystem(), rightCalibrated, leftCalibrated));
  ...
  private void setMMPosition(double targetInches) {
    if (m_climberValid) {
      // y = mx + b, where 0 degrees is 0.0 climber and 90 degrees is 1/4 winch turn
      // (the climber constant)
