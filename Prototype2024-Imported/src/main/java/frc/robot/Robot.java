...
public class Robot extends TimedRobot {
  ...
  public void teleopPeriodic() {
    ...
    if (m_controller.getAButtonPressed() || m_controller.getBButtonPressed()) {
      ...
      DataLogManager
          .log(String.format("%s button pressed - percent output: %.2f", (aButton) ? "A" : "B", m_fixedSpeed));
      ...
      DataLogManager
          .log(String.format("Start: goal: %.2f setpoint: %.2f Position: %.3f", m_goal.position, m_setpoint.position,
    ...
    switch (m_controlMode) {
      ...
      case kClosedLoop: {
        ...
        DataLogManager.log(
            String.format("Loop:  goal: %.2f setpoint: %.2f Position: %.3f", m_goal.position, m_setpoint.position,
                position));
        if (m_timer.hasElapsed(4.0) || Math.abs(position - m_goal.position) < 0.05) {
          DataLogManager
              .log(String.format("Return to open loop - %s", (m_timer.hasElapsed(4.0) ? "timed out" : "at goal")));
          m_controlMode = ControlMode.kStopped;
          m_percentOutput = 0.0;
        } else {
          DataLogManager.log(
